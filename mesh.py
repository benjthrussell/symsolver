...
class Mesh(object):
    ...
    *Arguments*
    ...
    def __init__( self, physics, r_min=0., r_max=None,
                  num_cells=800, too_fine=1e4, Ntol=1e-8,
        ...
        # mesh in units of source radius
        self.rs = 1.
        
        # extrema and number of cells
        self.r_min = r_min
        self.r_max = r_max
        if self.r_max is None:
            # if the field's Compton wavelength is outside of the source, default to 10^4 times it
            if self.physics.x0 < 1. :
                self.r_max = 1e4 / self.physics.x0
            else:
            # otherwise 10^2 the source radius is usually sufficient
                self.r_max = 1e2 * self.rs
