...
class QuadraticVev(Solver):
    
    def __init__( self, fem, source, physics,
                  mn=None, mf=None,
                  abs_dphi_tol=1e-16, rel_dphi_tol=1e-16, abs_res_tol=None, rel_res_tol=1e-16,
    ...
    def strong_residual_form( self, sol, units ):
        ...
        f = sol.dx(0).dx(0) + Constant(D-1.)/r * sol.dx(0) + (mu/mn)**2*sol \
            - lam*(mf/mn)**2*sol**3 -  mn**(D-2.)/M**2 * self.source.rho * sol
        f *= resc
    ...
    def linear_solver( self, phi_k ):
        ...
        a = - inner( grad(phi), grad(v) ) * rD * dx + ( (mu/mn)**2  \
            - 3.*lam*(mf/mn)**2*phi_k**2 - (mn**(D-2.)/M**2)*self.source.rho ) * phi * v * rD * dx
        L = (- 2.*lam*(mf/mn)**2*phi_k**3 ) * v * rD * dx
    ...
    def scalar_force( self ):
        ...
        force = - self.Phi * grad / Constant(self.physics.M**2)
    ...
    def compute_screening_factor( self ):
        ...
        screening_factor = self.Phi * self.Phi.dx(0) / (self.yukawa.dx(0) * self.physics.M)
    ...
    def compute_derrick( self ):
        ...
        # to avoid integrating numerical noise (and blowing it up by r^2) we restrict integration
        ...
        erho = self.source.rho * self.Phi**2 / ( 2. * M**2 )
        ...
        derrick2 = self.source.Rho_bar * self.source.Rs**D * \
                   self.Phi(self.fem.mesh.rs)**2 / (2. * self.physics.M**2 )
        derrick3 = self.source.Rho_bar * self.source.Rs**(D+1.) * \
                   self.Phi(self.fem.mesh.rs) * self.grad_Phi(self.fem.mesh.rs) / self.physics.M**2
