...
:synopsis: This module redefines FEniCS's built-in norm, errornorm and project functions 
           to adopt the :math:`r^2 dr` measure instead of the :math:`dx` measure. 

           Additionally, I define a :func:`get_values` function to fix a FEniCS bug that means
           cells in 1D meshes are sometimes not connected correctly in plots.
...
def rD_errornorm(u, uh, D=None, func_degree=None, norm_type="l2", degree_rise=3, mesh=None ):
    r"""
    ...
    # Compute norm
    return rD_norm(e, D, func_degree=func_degree, norm_type=norm_type, mesh=mesh )
...
def project(v, V=None, D=None, func_degree=None, bcs=None, mesh=None,
            function=None,
            solver_type="lu",
...
def get_values( function, output_mesh=False ):
    """
    This function implements a workaround for a bug in this version of FEniCS that means 
    sometimes cells in 1D meshes are not connected correctly in plots:
    ...
    This function substitutes the built-in compute_vertex_values().
